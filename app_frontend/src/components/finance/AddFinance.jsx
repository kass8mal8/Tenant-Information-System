...
const AddFinance = () => {
    ...
    const handleInputChange = (e) => {
        setFinanceDetails({
            ...financeDetails, [e.target.name]: e.target.value
        })
    }
    ...
    return (  
        <Stack direction='row' spacing={2} sx={{width: '95%', marginLeft: '0%', background: 'white', p: 3, borderRadius: '5px'}}>
            <form style={{ marginTop: '30px', width: '50%', marginRight: '10%' }} onSubmit={handleFinanceSubmit}>
                <Typography variant='body2' color='text.secondary' sx={{ my: 3 }} >Add new finance records!</Typography>
                    ...
                    <FormControl fullWidth>
                        ...
                        >
                            { appartmentData?.appartments.map( property => (
                                <MenuItem value={property.house_number} key={property._id}>{property.house_number}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                    ...
                    <FormControl fullWidth>
                        ...
                        >
                            { tenantData?.tenants.map( tenant => (
                                tenant.house_number === houseNumber && <MenuItem value={tenant.name} key={tenant._id}>{tenant.name}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                </Stack>
